...
iapp_mouse_id = np.where(adata_beta.var['human_name'] == 'IAPP')[0][0]
iapp_human_id = np.where(adata_ref.var.index == 'IAPP')[0][0]

print ('Iapp ID', iapp_mouse_id, iapp_human_id)

assert (np.all(adata_ref.obs['cell_type'].unique() == ['beta'])) # only beta cells mapping
assert (np.all(set(adata_ref.obs['disease'].unique()) == set(['control', 'T2D']))) # only ND and T2D and correct labels
...
assert (len(intersection_genes) > 0) # var_names is human name

data = adata_beta.to_df()
data.columns = adata_beta.var['human_name']
data_ref = adata_ref.to_df()

col_pairs =  np.array([[adata_beta.var['human_name'].tolist().index(col), ind] for ind, col in enumerate(adata_ref.var_names) if (col in adata_beta.var['human_name'].values)])

print ('len(col_pairs): {}'.format(col_pairs.shape[0]))
assert (col_pairs.shape[0] == len(intersection_genes))

intersection_genes_with_repeats = data[intersection_genes].columns
X = data[intersection_genes] - data[intersection_genes].mean(axis=0) + data_ref[intersection_genes_with_repeats].mean(axis=0)
...
lambda_correspondence = 15 # orig 0.1
#############################################
#############################################
##### tf graph
def tbn(name):
...
sess = tf.Session(config=build_config())
...
while training_counter < TRAINING_STEPS + 1:
    ...
    if training_counter in [100, 2500, TRAINING_STEPS]:
        col1, col2 = iapp_mouse_id, iapp_human_id # Iapp
        ...
        for i in [131,132,133]:
            ax.append(fig.add_subplot(i, projection='3d'))

        scprep.plot.scatter3d(pca_viz_x1, c=PCA1.inverse_transform(x1)[:, col1], title='All\n(tfx1)', ticks=None, ax=ax[0], fontsize=11, azim=-150)
        scprep.plot.scatter3d(pca_viz_x1, c='lightgray', ticks=None, ax=ax[1], fontsize=11, azim=-150, alpha=0.1)
        scprep.plot.scatter3d(pca_viz_fake1, c=PCA1.inverse_transform(eval_fake1)[:, col1], title='Fake All\n(fake1) Iapp', ticks=None, ax=ax[1], fontsize=11, azim=-150)
        scprep.plot.scatter3d(pca_viz_x1, c='lightgray', ticks=None, ax=ax[2], fontsize=11, azim=-150, alpha=0.1)
        scprep.plot.scatter3d(pca_viz_fake1, c=PCA2.inverse_transform(x2_final)[:, col2], title='Fake All, colored\nreal 4m (tfx2) Iapp', ticks=None, ax=ax[2], fontsize=11, azim=-150)
        ...
        fig.savefig(f'/home/av622/project/islet_human_atlas/all_plot_{sample}.png')
        print('Plot saved.')

        print ('Iapp correlation(fake1, tfx2)', scipy.stats.spearmanr(PCA1.inverse_transform(eval_fake1)[:, col1], PCA2.inverse_transform(x2_final)[:, col2]).correlation)
        ...
        output_fake1_ambient = PCA1.inverse_transform(output_fake1)
        output_fake1_ambient = output_fake1_ambient #* 10

        with open(f'results/output_human_all_to_all_cycle_{lambda_cycle}_correspondence_{lambda_correspondence}_corr_correspondence_{sample}_normal_t2d.npz', 'wb+') as f:
            np.savez(f, sample_to_all=output_fake1_ambient, sample_to_all_pc=output_fake1)
