...
intersection_genes = np.array(list(set(adata_beta.var_names).intersection(stz.var_names)))

adata_beta_wt = adata_beta[adata_beta.obs['samples'] == 'WT']
wt_magic_op = magic.MAGIC(random_state=42, t=10, verbose=False)
data = wt_magic_op.fit_transform(adata_beta_wt.to_df())
wt_genes = np.array(adata_beta_wt.var_names)

stz_ctrl = stz[stz.obs.treatment == 'Ctrl']
magic_op = magic.MAGIC(random_state=42, t=10, verbose=False)
data_ref = magic_op.fit_transform(stz_ctrl.to_df())
ctrl_genes = np.array(stz_ctrl.var_names)

col_pairs =  np.array([[wt_genes.tolist().index(col), ind] for ind, col in enumerate(ctrl_genes) if (col in wt_genes)])
...
while training_counter < TRAINING_STEPS + 1:
    ...
    if training_counter in [100, 5000, 7500, 10000, 12500, TRAINING_STEPS]:
        col1, col2 = 15365, 13722 # Ucn3

        eval_fake1 = get_layer(sess, tbn('x2:0'), x2_final, tbn('fake1:0'))
        pca_viz_fake1 = pca_viz1.transform(eval_fake1)

        print ('Ucn3 correlation(fake1, tfx2)', scipy.stats.spearmanr(PCA1.inverse_transform(eval_fake1)[:, col1], PCA2.inverse_transform(x2_final)[:, col2]).correlation)
        ...
        with open(f'results/output_stz_to_wt_cycle_1_correspondence_lambda_10_mse_correspondence_training_counter_{training_counter}.npz', 'wb+') as f:
            np.savez(f, stz_to_wt=output_fake1)
