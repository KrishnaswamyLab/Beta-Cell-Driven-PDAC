...
obob_magic_op = magic.MAGIC(random_state=42, t=10, verbose=False)
data = obob_magic_op.fit_transform(adata_beta_obob.to_df())
obob_genes = np.array(adata_beta_obob.var_names)

stz_treatment = stz[stz.obs.treatment == 'Vehicle-STZ']
magic_op = magic.MAGIC(random_state=42, t=10, verbose=False)
data_ref = magic_op.fit_transform(stz_treatment.to_df())
treatment_genes = np.array(stz_treatment.var_names)

col_pairs =  np.array([[obob_genes.tolist().index(col), ind] for ind, col in enumerate(treatment_genes) if (col in obob_genes)])
...
while training_counter < TRAINING_STEPS + 1:
    ...
    if training_counter in [100, 5000, 7500, 10000, 12500, TRAINING_STEPS]:
        ...
        with open(f'results/output_stz_to_obob_cycle_1_correspondence_lambda_10_corr_correspondence_training_counter_15000.npz', 'wb+') as f:
            np.savez(f, stz_to_obob=output_fake1)
