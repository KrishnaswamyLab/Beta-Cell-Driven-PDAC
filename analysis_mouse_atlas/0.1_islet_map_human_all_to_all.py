...
col_pairs =  np.array([[adata_beta.var_names.tolist().index(col), ind] for ind, col in enumerate(adata_ref.var_names) if (col in adata_beta.var_names.values)])
...
x1_raw_nonuniform = data.values.astype(np.float32)
x2_raw_nonuniform = data_ref.values.astype(np.float32)
...
while training_counter < TRAINING_STEPS + 1:
    ...
    if training_counter in [100, 2500, TRAINING_STEPS]:
        ...
        with open(f'results/output_mouse_normal_to_all_cycle_{lambda_cycle}_correspondence_{lambda_correspondence}_corr_correspondence_{sample}_training_counter_normal_t2d.npz', 'wb+') as f:
