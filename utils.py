...
def get_cells_of_origin(trajectories, data_pc, timepoint=-1):
    # get distance to real cells in number of PCs used by TrajectoryNet, default -1 to get cell-of-origin
    dist = cdist(trajectories[timepoint, :, :], data_pc[:, :trajectories.shape[-1]]) 
    # get nearest real cell for each generated cell
    nearest_cells = np.argsort(dist, axis=1)[:, 0] 
    return (nearest_cells)

def plot_heatmap(genes, values_scaled, savefig='figures/tmp.png'):
    df = values_scaled.T[genes]

    clx = sns.clustermap(df.T, cmap="inferno", robust=True, col_cluster=False, row_cluster=False)
    ax = clx.ax_heatmap
    ax.set_xticks([10, 40, 70])
    ax.set_xticklabels(["WT", "HFD", "$Lep^{ob/ob}$"])
    names = genes
    clx.savefig(savefig, dpi=200)
...
def sinkhorn_knopp_unbalanced(
    ...
    **kwargs
    ...
    Code from TrajectoryNet (Tong et al).
